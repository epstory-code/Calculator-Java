...
public class CalculatorEngine {
    ...
    public String[] evaluateExpression(String expression) {
        ...
        for (String x : list) {
            index++;
            if (!(x.equals(mult) | x.equals(div) | x.equals(add) | x.equals(sub) | x.equals(rem) | x.equals("Undefined") | x.equals(error))) {
        ...
        if (!(list.contains("Undefined") | list.contains(error) | list.contains("That button doesnt work child"))) {
            index = 0;
            while (list.size() > 1) {
                System.out.println(list);
